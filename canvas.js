...
const c  = canvas.getContext('2d') // 2d 게임

canvas.width = innerWidth
canvas.height = innerHeight

const scoreEl = document.querySelector('#scoreEl')          // 게임 내 점수 
const startBtn = document.querySelector('#startBtn')        // 시작 버튼
const modalEl = document.querySelector('#modalEl')          // 게임 시작시 작은 창
const bigScoreEl = document.querySelector('#bigScoreEl')    // 마지막 최종 점수
...
let player = new Player(x, y, 10, 'white') // 플레이어 생성 
let projectiles = []
let enemies = []
...
function spawnEnemies(){
    setInterval(() => {
        const radius = Math.random() * (30 - 4) + 4 // 4 ~ 30 사이 값의 크기를 가지는 적 생성
